...
import { Box, HStack, Center, Text, Input, FormControl, NumberInput, NumberInputField, VStack, Divider } from "@chakra-ui/react";
...
import {
    ...
    request_DM_Manager_data,
    request_DM_data,
    serialise_DM_Mint_instruction,
    serialise_basic_instruction,
    request_DM_User_data,
    run_keyData_GPA,
...
export function DMScreen({ bearer_token }: { bearer_token: string }) {
    ...
    const check_dm_nft = useCallback(async () => {
        if (current_dm_nft_meta.current === null) return;
    ...
    const check_dm_state = useCallback(async () => {
        ...
        if (check_state.current === true) {
            if (DEBUG) console.log("checking DM manager state");
            ...
            if (dm_index === null || current_fees === null) check_state.current = false;
    ...
    const Burn = useCallback(async () => {
        ...
        try {
            ...
            if (DEBUG) console.log(transaction_response);

            if (DEBUG) console.log("sig: ", signature);
        } catch (error) {
            console.log(error);
            return;
        }
    ...
    const GetFees = useCallback(async () => {
        ...
        if (wallet.publicKey.toString() !== FOUNDER_1_KEY.toString() && wallet.publicKey.toString() !== FOUNDER_2_KEY.toString()) {
            if (dm_name === "" || dm_mint === null) return;
    ...
    const CheckMemberStatus = useCallback(async () => {
        ...
        if (DEBUG) console.log("dm name", dm_name);
    ...
    function FoundersDialogue() {
        ...
        if (founder_excess === undefined) return <></>;

        return (
            <Box width="80%">
                <Center>
                    <div className="font-face-sfpb">
                        <Text mb="2rem" fontSize={DUNGEON_FONT_SIZE} textAlign="center" color="white">
                            {" "}
                            Welcome Founder! Please help yourself to our records and your share of the guilds proceeds
                        </Text>
                        <Divider />
                        <Text mt="2rem" fontSize={DUNGEON_FONT_SIZE} textAlign="center" color="white">
                            {" "}
                            Total Fees: {current_fees}
                        </Text>
                        <Text mb="2rem" fontSize={DUNGEON_FONT_SIZE} textAlign="center" color="white">
                            {" "}
                            To Claim: {founder_excess.toFixed(5)}
                        </Text>
                        <Center>
    ...
    function ApplicantsJourneyFlavourText({ keys_burnt }: { keys_burnt: number | null }) {
        ...
        return (
            <Text fontSize={DUNGEON_FONT_SIZE} textAlign="center" color="white">
                {" "}
                {ApplicantsJourneyFlavourTextArray[keys_burnt]}
            </Text>
        );
    }
                ...
                <Center>
                    <VStack spacing="1rem">
                        ...
                        <div className="font-face-sfpb">
                            <Text fontSize={DUNGEON_FONT_SIZE} textAlign="center" color="white">
                                {" "}
                                Choose your sacrifice ({keys_burnt
                                    ? Math.min(keys_burnt + 1 + keys_to_burn, 10)
                                    : 1 + keys_to_burn}/10){" "}
                            </Text>
                        </div>
                        <HStack>
    ...
    function Dialogue() {
        ...
        if (DEBUG) console.log("Member Status: ", member_status);
                ...
                <Box width="100%">
                    <Center>
                        <div className="font-face-sfpb">
                            <Text mt="2rem" fontSize={DUNGEON_FONT_SIZE} textAlign="center" color="white">
                                {" "}
                                If you are member you need only speak your name.
                            </Text>
                            <Text mb="2rem" fontSize={DUNGEON_FONT_SIZE} textAlign="center" color="white">
                                {" "}
                                If you are an applicant stay silent.
                            </Text>
                            <Center>
                            ...
                            <Center>
                                <VStack>
                                    ...
                                    {dm_error != null && (
                                        <Text fontSize={DUNGEON_FONT_SIZE} textAlign="center" color="red">
                                            {" "}
                                            {dm_error}{" "}
                                        </Text>
                                    )}
        ...
        if (member_status === MemberStatus.member) {
            ...
            if (this_member_excess === undefined) return <></>;
                    ...
                    <Center>
                        <div className="font-face-sfpb">
                            <VStack>
                                ...
                                <Text fontSize={DUNGEON_FONT_SIZE} textAlign="center" color="white">
                                    {" "}
                                    Please help yourself to our records and your share of the guilds proceeds
                                </Text>
                                <Divider />
                                <Text fontSize={DUNGEON_FONT_SIZE} textAlign="center" color="white">
                                    {" "}
                                    Total Guild Fees Raised: {current_fees.toFixed(5)}
                                </Text>
                                <Text fontSize={DUNGEON_FONT_SIZE} textAlign="center" color="white">
                                    {" "}
                                    Your Total Fees Withdrawn: {dm_fees_raised.toFixed(5)}
                                </Text>

                                <Text fontSize={DUNGEON_FONT_SIZE} textAlign="center" color="white">
                                    {" "}
                                    Your Outstanding Share: {this_member_excess.toFixed(5)}
                                </Text>
        ...
        if (DEBUG) console.log("Applicant: ", keys_burnt);
                    ...
                    <Center>
                        <Box width="100%">
                            <div className="font-face-sfpb">
                                <Text fontSize={DUNGEON_FONT_SIZE} textAlign="center" color="white">
                                    {" "}
                                    New applications to the Guild will be accepted in the near future.{" "}
                                </Text>
                            </div>
                        </Box>
                    </Center>
                            ...
                            <Center>
                                <Box width="100%">
                                    <div className="font-face-sfpb">
                                        <Text fontSize={DUNGEON_FONT_SIZE} textAlign="center" color="white">
                                            {" "}
                                            It seems you have friends in high places. Very well. Proceed.{" "}
                                        </Text>
                                    </div>
                                </Box>
                            </Center>
                                    ...
                                    <Center>
                                        <Box width="100%">
                                            <div className="font-face-sfpb">
                                                <Text fontSize={DUNGEON_FONT_SIZE} textAlign="center" color="white">
                                                    {" "}
                                                    Your journey is complete. The life you have known is now over, and a new one begins as a
                                                    member of the Dungeon Masters guild.{" "}
                                                </Text>
                                            </div>
                                        </Box>
                                    </Center>
                                            ...
                                            <div className="font-face-sfpb">
                                                <Text fontSize={DUNGEON_FONT_SIZE} textAlign="center" color="white">
                                                    ...
                                                    <a
                                                        ...
                                                        target="_blank"
                                                        rel="noreferrer"
                                                        href={"https://explorer.solana.com/address/" + current_dm_mint.toString()}
                                                    >
                                                        here
                                                    </a>
                                                </Text>
