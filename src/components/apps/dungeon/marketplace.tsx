...
import {run_marketplace_GPA, ListingData, bignum_to_num, get_current_blockhash, send_transaction, uInt32ToLEBytes, serialise_Marketplace_list_instruction, serialise_Marketplace_buy_instruction} from './utils';
...
export function MarketplaceScreen({bearer_token} : {bearer_token : string})
{
    ...
    const ListingCard = ({listing, index} : {listing : ListingData, index : number}) => {
        ...
        return (
            <tr>
                <td><img src={chicken} width="auto" alt={""} style={{maxHeight: EMOJI_SIZE, maxWidth: EMOJI_SIZE}}/></td>
                <td >Test Item</td>
                <td >{listing.quantity}</td>
                <td >{unit_price}</td>
                <td> 
                    <NumberInput 
                        defaultValue={listing.quantity} min={0} max={listing.quantity}
    ...
    const BuyItemOnMarketplace = useCallback( async (index : number) => 
    {
                      ...
                      <div className="font-face-sfpb">
                        <VStack align="center">
                          ...
                          <HStack width="80%" align={"left"}>
                            ...
                            <Box width="50%">
                              <NumberInput
                                ...
                                value={
                                  list_quantity >= 0 ? list_quantity.toString() : ""
                                }
                                precision={0}
                                borderColor="white"
                                min={0}
                          ...
                          <Box
                            ...
                            width="60px"
                          >
                            <Text align="center" onClick={ListItemOnMarketplace} fontSize={DUNGEON_FONT_SIZE} color="white">
                              LIST
                            </Text>
                          </Box>
                        </VStack>
