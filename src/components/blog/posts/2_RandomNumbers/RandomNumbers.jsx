...
unsafe fn any_as_u8_slice<T: Sized>(p: &T) -> &[u8] {
    ...
    <div className="home">
        <div class="container">
            ...
            In this post we will focus on a different class of problems where the random values aren't required within the same block or transaction that the user interacts with your program.  A simple example of such a program is a group lottery or raffle, where many users submit bids to win, and winners are chosen in blocks produced after the raffle has closed.  In this use case it is impossible for any bidder to influence the random numbers chosen, and therefore unfairly increase their chances of winning. All we need to do is maintain the state for an RNG on the block chain, and update it whenever we need a new value. While the program owner will still need to provide some initial seed, after that the program will generate new values independently as needed, with the owner no longer having any way to influence what numbers are generated, so it can also be trustless.
